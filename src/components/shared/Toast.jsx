...
const Toast = ({ toast, onRemove }) => {
  ...
  const getIcon = () => {
    switch (toast.type) {
      ...
      case "info":
        return <Info className="h-5 w-5 text-slate-900" />;
      default:
        return null;
    }
  };
  ...
  const getToastStyles = () => {
    ...
    switch (toast.type) {
      ...
      case "info":
        return `${baseStyles} border-slate-200 bg-white text-slate-950`;
      default:
      ...
      <div className={getToastStyles() + " py-4 px-5"}>
        <div className="flex items-start space-x-4 w-full">
          ...
          <button
            ...
            className="ml-2 rounded-md p-1 text-gray-400 hover:text-gray-700 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:ring-offset-2 transition-colors"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
